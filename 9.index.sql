-- pk, fk, unizque제약조건 추가 시에 해당 컬럼에 대해 index 페이지 자동생성
-- index가 만들어지면, 조회성능은 향상. 추가/수정/삭제 성능은 하락
...
-- index를 통해 조회성능향상을 얻으려면, 반드시 where 조건에 해당 컬럼에 대한 조건이 있어야 함.
select * from auhthor where email="honggildong@naver.com";

-- 만약 where 조건에서 2컬럼으로 조회시에, 1컬럼에만 index가 있다면. 
select * from author where id=10 and email="honggildong@naver.com"

-- 만약 where조건에서 2컬럼으로 조회시에, 2컬럼 각각 index가 있다면.
-- 이 경우 db엔진에서 최적의 알고리즘 실행.
select * from author where name='hong' and email="honggildong@naver.com"
...
-- index테스트 시나리오
-- 아래 프로시저를 통해 수십만건의 데이터 insert 후에 index 생성 전 후에 따라 조회성능확인
DELIMITER //
